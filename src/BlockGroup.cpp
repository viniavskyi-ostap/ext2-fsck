...
void BlockGroup::additionalFieldsCheck() {
    ...
    uint64_t group_desc_offset = block_group_i == 0 ?
                                 (image.block_size == BOOT_SIZE ? 2 * image.block_size : image.block_size) :
                                 ((image.blocks_per_group * block_group_i + 1) * image.block_size);
    image.istream.seekg(group_desc_offset, std::ios::beg);
    image.istream.read((char*) &group_desc, sizeof(group_desc));

    uint32_t block_bitmap_i = group_desc.bg_block_bitmap;
    uint32_t inode_bitmap_i = group_desc.bg_inode_bitmap;
    ...
    if (image.block_usage[block_bitmap_i])
        errors.push_back("Block bitmap References already used block " + std::to_string(block_bitmap_i));
    if (image.block_usage[inode_bitmap_i])
        errors.push_back("Inode bitmap References already used block " + std::to_string(inode_bitmap_i));
    if (!errors.empty()) return;
    ...
    for (uint32_t i = 0; i < image.blocks_per_group; ++i) {
        uint32_t block_i = image.blocks_per_group * block_group_i + i;
        ...
        if (stored) ++block_count;
    }

//    std::cout << group_desc.bg_free_blocks_count << std::endl;
    if (group_desc.bg_free_blocks_count != image.blocks_per_group - block_count)
        errors.push_back("Incorrect field of free blocks count");
